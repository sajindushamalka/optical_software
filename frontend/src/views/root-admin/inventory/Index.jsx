...
const RootAdminInventory = () => {
    ...
    const fetchPurpose = async () => {
        try {
            const res = await axios.get("http://localhost:2776/api/root/doctor");
    ...
    const handleSave = async () => {
        try {
            if (editId) {
                // Update
                await axios.put(`http://localhost:2776/api/root/doctor/${editId}`, {
                    text: newText,
                });
            } else {
                // Create
                await axios.post("http://localhost:2776/api/root/doctor", {
                    text: newText,
                });
            }
            setShowModal(false);
    ...
    const handleDelete = async (id) => {
        if (window.confirm("Are you sure you want to delete this record?")) {
            try {
                await axios.delete(`http://localhost:2776/api/root/doctor/${id}`);
                fetchPurpose();
            } catch (err) {
                console.error(err);
            }
        ...
        <React.Fragment>
                                        ...
                                        <tr key={p.u_id}>
                                            ...
                                            <td>
                                                ...
                                                <Button
                                                    variant="danger"
                                                    size="sm"
                                                    onClick={() => handleDeleteusers(p.u_id)}
                                                >
                                                    <FaTrash />
                                                </Button>
                                            </td>
            ...
            <Modal show={showModal} onHide={() => setShowModal(false)}>
                <Modal.Header closeButton>
                    <Modal.Title>
                        {editId ? "Edit Doctor Rx" : "Create New Doctor Rx"}
                    </Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <Form>
...
export default RootAdminInventory;
